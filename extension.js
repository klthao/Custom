    ...
    function extend() {
		...
		bot.commands.randomCommand = {
			...
			functionality: function (chat, cmd) {
				...
				//	var randomInt = Math.floor((Math.randomInt() * bot.room.users.length) + 1)
				//	var randomUser = bot.room.users[randomInt].username;
				//	API.sendChat("@" + randomUser + "Hello");
					var roomUsers = [];
					...
					randomUser = roomUsers[randomUserPos];
